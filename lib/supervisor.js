...
function spawnHidden(bin, args, opts) {
  if (os.platform() !== "win32") return spawn(bin, args, opts);

  const esc = (s) => String(s).replace(/'/g, "''");
  const argStr = args.join(" ");
  const ps =
    `try {` +
    `$p = Start-Process -FilePath '${esc(bin)}' -ArgumentList '${esc(argStr)}' -WindowStyle Hidden -WorkingDirectory '${esc(opts.cwd || "")}' -PassThru;` +
    `Write-Output $p.Id;` +
    `$p.WaitForExit();` +
    `exit $p.ExitCode` +
  ...
  const proxy = {
    ...
    killed: false,
    exitCode: null,
    stdout: { on: () => {}, removeListener: () => {} },
    stderr: { on: () => {}, removeListener: () => {} },
    stdin: { write: () => {}, end: () => {} },
    _closeFn: null,
    _exitFn: null,
    _pidCb: null,
    _onPid(cb) { this._pidCb = cb; },
    on(ev, fn) { if (ev === "close") this._closeFn = fn; if (ev === "exit") this._exitFn = fn; return this; },
    once(ev, fn) { if (ev === "close") this._closeFn = fn; if (ev === "exit") this._exitFn = fn; return this; },
    kill(sig) {
      if (this.pid) { try { kill(this.pid, sig || "SIGKILL", () => {}); } catch {} }
  ...
  child.stdout.on("end", () => {
    const realPid = parseInt(pidBuf.trim().split(/\r?\n/)[0], 10);
    if (realPid) {
      proxy.pid = realPid;
      if (proxy._pidCb) proxy._pidCb(realPid);
    }
  });
  child.on("close", (code) => {
    proxy.exitCode = code;
...
async function startWorld(worldId) {
  ...
  // Update PID once resolved (spawnHidden resolves it asynchronously)
  if (child._onPid) child._onPid((pid) => dbm.updateWorld(worldId, { process_id: pid }));
