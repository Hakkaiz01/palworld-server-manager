...
// windowsHide=true. We route through PowerShell's Start-Process with
// -WindowStyle Hidden -NoNewWindow to suppress it completely.
// stdout/stderr are piped through named pipes so the parent can read them.
function spawnHidden(bin, args, opts) {
  ...
  const psScript = `Start-Process -FilePath "${esc(bin)}" -ArgumentList '${argStr}' -NoNewWindow -WindowStyle Hidden -PassThru -RedirectStandardOutput "CONOUT$" -RedirectStandardError "CONOUT$" | Select-Object -ExpandProperty Id`;
