...
// ---- Hidden spawn for Windows (no console window) ----
// PalServer.exe's launcher calls AllocConsole() even when spawned with
// windowsHide=true. We use Start-Process -WindowStyle Hidden to suppress
// the console window, and Wait-Process to keep PowerShell alive so the
// proxy's close handler fires when the actual server exits.
function spawnHidden(bin, args, opts) {
  if (os.platform() !== "win32") {
    return spawn(bin, args, opts);
  }

  const esc = (s) => String(s).replace(/"/g, '\\"').replace(/'/g, "''");
  const argStr = args.map((a) => `'${esc(a)}'`).join(" ");

  // Start-Process -WindowStyle Hidden suppresses the console.
  // Wait-Process blocks until the server exits, so the proxy's
  // close handler fires with the real exit code.
  const psScript =
    `$p=Start-Process -FilePath "${esc(bin)}" -ArgumentList ${argStr} -WindowStyle Hidden -PassThru;` +
    `$p.Id;` +
    `$p.WaitForExit();` +
    `exit $p.ExitCode`;
  ...
  // Read the PID from PowerShell's first stdout line
  let pidBuf = "";
  child.stdout.on("data", (d) => { pidBuf += d.toString(); });

  // Proxy object that mimics the child process API.
  // stdout/stderr from the actual server are captured via REST API
  // and chat file tailer, so dummy streams are fine here.
  const proxy = {
    pid: null,
    killed: false,
    exitCode: null,
    stdout: { on: () => {}, off: () => {}, removeListener: () => {} },
    stderr: { on: () => {}, off: () => {}, removeListener: () => {} },
    stdin: { write: () => {}, end: () => {} },
    _closeFn: null,
    _exitFn: null,
    on: function (ev, fn) {
      if (ev === "close") this._closeFn = fn;
      if (ev === "exit") this._exitFn = fn;
      return this;
    },
    once: function (ev, fn) {
      if (ev === "close") this._closeFn = fn;
    ...
    kill: function (sig) {
      if (this.pid) {
        try { kill(this.pid, sig || "SIGKILL", () => {}); } catch {}
      }
  ...
  // Resolve the real PID from PowerShell's first output line
  child.stdout.on("end", () => {
    const lines = pidBuf.trim().split(/\r?\n/);
    const realPid = parseInt(lines[0], 10);
    if (realPid) proxy.pid = realPid;
  });
  child.on("close", (code) => {
    proxy.exitCode = code;
    if (proxy._closeFn) proxy._closeFn(code);
    if (proxy._exitFn) proxy._exitFn(code);
  });
...
async function startWorld(worldId) {
  ...
  // Spawn options. On Windows, PalServer.exe's launcher calls AllocConsole()
  // even with windowsHide=true, so we use a PowerShell wrapper to suppress it.
  const spawnOpts = {
  ...
  const child = os.platform() === "win32"
    ? spawnHidden(bin, args, spawnOpts)
