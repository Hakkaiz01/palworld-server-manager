...
// windowsHide=true. We use cmd.exe /c start /b which starts the process
// in the background without creating any window. The /b flag is key —
// it suppresses the console that PalServer.exe would otherwise create.
function spawnHidden(bin, args, opts) {
  ...
  const cmd = `start /b /wait ${esc(bin)} ${args.map((a) => esc(a)).join(" ")}`;
