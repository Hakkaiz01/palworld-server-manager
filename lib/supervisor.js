...
// windowsHide=true. We use .NET's Process class via PowerShell with
// CreateNoWindow=true to suppress it completely.
function spawnHidden(bin, args, opts) {
  ...
  // .NET Process class with CreateNoWindow=true is the only reliable way
  // to suppress AllocConsole() in Unreal Engine applications
  const psScript =
    `$p=New-Object System.Diagnostics.Process;` +
    `$p.StartInfo.FileName="${esc(bin)}";` +
  ...
  // Read the PID from PowerShell's stdout
  let pidBuf = "";
  child.stdout.on("data", (d) => { pidBuf += d.toString(); });

  // Proxy object that mimics the child process API.
  // stdout/stderr from the child are captured by .NET internally;
  // we use dummy streams since the existing log capture (REST API,
  // chat file tailer) provides the actual server output.
  const proxy = {
    pid: null,
    killed: false,
    exitCode: null,
  ...
  // Resolve the real PID once PowerShell outputs it
  child.stdout.on("end", () => {
    const realPid = parseInt(pidBuf.trim(), 10);
