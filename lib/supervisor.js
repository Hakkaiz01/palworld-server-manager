...
function spawnHidden(bin, args, opts) {
  ...
  // node-pty returns a PTY handle, not a child_process.
  // Wrap it in a proxy that mimics the child_process API.
  const proxy = {
    pid: term.pid,
    killed: false,
    exitCode: null,
    stdout: { on: (ev, fn) => { if (ev === "data") term.onData(fn); }, removeListener: () => {} },
    stderr: { on: () => {}, removeListener: () => {} },
    stdin: { write: (d) => term.write(d), end: () => {} },
    _closeFn: null,
    _exitFn: null,
    on(ev, fn) {
      if (ev === "close") this._closeFn = fn;
      if (ev === "exit") this._exitFn = fn;
      if (ev === "data") term.onData(fn);
      return this;
    },
    once(ev, fn) {
      if (ev === "close") this._closeFn = fn;
    ...
    kill(sig) {
      try { term.kill(sig || "SIGKILL"); } catch {}
      this.killed = true;
    },
  ...
  term.onExit(({ exitCode }) => {
    proxy.exitCode = exitCode;
    if (proxy._closeFn) proxy._closeFn(exitCode);
    if (proxy._exitFn) proxy._exitFn(exitCode);
  });
